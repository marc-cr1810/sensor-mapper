...
public:
  ...
  static auto render_pattern_selector(
      std::shared_ptr<antenna_pattern_t>& current_pattern,
      antenna_pattern_ui_state_t& state) -> bool {
    ...
    if (ImGui::BeginCombo("##pattern_combo", pattern_name)) {
      ...
      if (!state.recent_patterns.empty()) {
        ...
        for (const auto& pattern : state.recent_patterns) {
          if (pattern && ImGui::Selectable(pattern->name.c_str(), 
  ...
  static auto render_pattern_library_dialog(antenna_pattern_ui_state_t& state) 
  ...
  static auto render_pattern_editor(std::shared_ptr<antenna_pattern_t>& pattern,
                                    antenna_pattern_ui_state_t& state) -> void {
    if (!pattern) return;
    ...
    if (ImGui::Begin("Pattern Editor", &state.show_pattern_editor)) {
      ...
      // Polarization
      const char* pol_types[] = {
        "Vertical", "Horizontal", "Circular Left", "Circular Right", "Dual Slant", "Unknown"
      };
  ...
  static auto render_pattern_visualizer(std::shared_ptr<antenna_pattern_t>& pattern,
                                       antenna_pattern_ui_state_t& state) -> void {
    if (!pattern) return;
  ...
  // File load dialog
  static auto render_load_dialog(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    ...
    if (ImGui::Begin("Load Antenna Pattern", &state.show_load_dialog)) {
      ...
      if (ImGui::Button("Load")) {
        auto result = antenna_pattern_io_t::load_from_file(state.filepath_buffer, 
  ...
  // File save dialog
  static auto render_save_dialog(antenna_pattern_ui_state_t& state,
                                 const antenna_pattern_t& pattern) -> bool {
    bool saved = false;
    ...
    if (ImGui::Begin("Save Antenna Pattern", &state.show_save_dialog)) {
      ...
      state.selected_format = (format_idx == 1) ? antenna_file_format_t::JSON : 
                                                  antenna_file_format_t::CSV;
      
      if (ImGui::Button("Save")) {
        auto result = antenna_pattern_io_t::save_to_file(pattern, 
                                                         state.filepath_buffer,
                                                         state.selected_format);
        if (result.success) {
          saved = true;
        } else {
...
private:
  ...
  static auto add_to_recent(antenna_pattern_ui_state_t& state,
                           std::shared_ptr<antenna_pattern_t> pattern) -> void {
  ...
  static auto render_basic_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    
    const char* types[] = { "Omnidirectional", "Isotropic", "Dipole", "Sector", 
                           "Patch", "Yagi", "Horn", "Parabolic", "Bi-directional" };
    ...
    switch (state.selected_pattern_type) {
      ...
      case 4: // Patch
        ...
        preview_pattern = antenna_library_t::generate_patch(state.param_beamwidth_h, 
                                                            state.param_beamwidth_v,
                                                            state.param_gain_dbi);
        if (ImGui::Button("Generate Patch")) {
          pattern = antenna_library_t::generate_patch(state.param_beamwidth_h, 
                                                      state.param_beamwidth_v,
                                                      state.param_gain_dbi);
        }
        break;
      case 5: // Yagi
        ...
        preview_pattern = antenna_library_t::generate_yagi(state.param_beamwidth_h, 
                                                           state.param_num_elements);
        if (ImGui::Button("Generate Yagi")) {
          pattern = antenna_library_t::generate_yagi(state.param_beamwidth_h, 
                                                     state.param_num_elements);
        }
        break;
      case 6: // Horn
        ...
        preview_pattern = antenna_library_t::generate_horn(state.param_beamwidth_h, 
                                                           state.param_gain_dbi);
        if (ImGui::Button("Generate Horn")) {
          pattern = antenna_library_t::generate_horn(state.param_beamwidth_h, 
                                                     state.param_gain_dbi);
        }
        break;
      case 7: // Parabolic
        ...
        preview_pattern = antenna_library_t::generate_parabolic(state.param_diameter_m, 
                                                                state.param_frequency_mhz);
        if (ImGui::Button("Generate Parabolic")) {
          pattern = antenna_library_t::generate_parabolic(state.param_diameter_m, 
                                                          state.param_frequency_mhz);
        }
        break;
  ...
  static auto render_cellular_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    switch (state.selected_pattern_type) {
      case 0: // Sector
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_cellular_sector_antenna(
            state.param_frequency_mhz, state.param_electrical_tilt);
        if (ImGui::Button("Generate Cellular Sector")) {
          pattern = antenna_pattern_library_advanced_t::generate_cellular_sector_antenna(
              state.param_frequency_mhz, state.param_electrical_tilt);
        }
        break;
      case 1: // Small Cell
        preview_pattern = antenna_pattern_library_advanced_t::generate_small_cell_omni(
            state.param_frequency_mhz);
        if (ImGui::Button("Generate Small Cell Omni")) {
          pattern = antenna_pattern_library_advanced_t::generate_small_cell_omni(
              state.param_frequency_mhz);
        }
        break;
      case 2: // MIMO
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_massive_mimo_array(
            state.param_num_elements, v_elements, state.param_frequency_mhz);
        if (ImGui::Button("Generate Massive MIMO")) {
          pattern = antenna_pattern_library_advanced_t::generate_massive_mimo_array(
              state.param_num_elements, v_elements, state.param_frequency_mhz);
        }
        break;
    }
  ...
  static auto render_wifi_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    switch (state.selected_pattern_type) {
      ...
      case 1: // Patch
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_wifi_patch_panel(
            state.param_beamwidth_h);
        if (ImGui::Button("Generate WiFi Patch")) {
          pattern = antenna_pattern_library_advanced_t::generate_wifi_patch_panel(
              state.param_beamwidth_h);
        }
        break;
    }
  ...
  static auto render_iot_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    switch (state.selected_pattern_type) {
      case 0: // LoRa
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_lora_gateway_collinear(
            state.param_num_elements);
        if (ImGui::Button("Generate LoRa Gateway")) {
          pattern = antenna_pattern_library_advanced_t::generate_lora_gateway_collinear(
              state.param_num_elements);
        }
        break;
      case 1: // Chip
  ...
  static auto render_satellite_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    switch (state.selected_pattern_type) {
      ...
      case 1: // Dish
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_satellite_dish(
            state.param_diameter_m, state.param_frequency_mhz / 1000.0f);
        if (ImGui::Button("Generate Satellite Dish")) {
          pattern = antenna_pattern_library_advanced_t::generate_satellite_dish(
              state.param_diameter_m, state.param_frequency_mhz / 1000.0f);
        }
        break;
    }
  ...
  static auto render_p2p_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    auto preview_pattern = antenna_pattern_library_advanced_t::generate_microwave_backhaul(
        state.param_diameter_m, state.param_frequency_mhz / 1000.0f);
    
    if (ImGui::Button("Generate Microwave Backhaul")) {
      pattern = antenna_pattern_library_advanced_t::generate_microwave_backhaul(
          state.param_diameter_m, state.param_frequency_mhz / 1000.0f);
    }
  ...
  static auto render_radar_patterns(antenna_pattern_ui_state_t& state) 
      -> std::shared_ptr<antenna_pattern_t> {
    std::shared_ptr<antenna_pattern_t> pattern;
    ...
    switch (state.selected_pattern_type) {
      case 0: // Phased Array
        ...
        ImGui::InputFloat("Frequency (GHz)", &state.param_frequency_mhz, 1.0f, 10.0f);
        preview_pattern = antenna_pattern_library_advanced_t::generate_phased_array_radar(
            state.param_num_elements, state.param_frequency_mhz / 1000.0f, 0.0f);
        if (ImGui::Button("Generate Phased Array")) {
          pattern = antenna_pattern_library_advanced_t::generate_phased_array_radar(
              state.param_num_elements, state.param_frequency_mhz / 1000.0f, 0.0f);
        }
        break;
      case 1: // Beamformed
        ...
        preview_pattern = antenna_pattern_library_advanced_t::generate_beamformed_pattern(
            state.param_beamwidth_h, state.param_beamwidth_v, 15.0f, state.param_num_elements);
        if (ImGui::Button("Generate Beamformed Pattern")) {
          pattern = antenna_pattern_library_advanced_t::generate_beamformed_pattern(
              state.param_beamwidth_h, state.param_beamwidth_v, 15.0f, state.param_num_elements);
        }
        break;
    }
  ...
  // Render polar plot
  static auto render_polar_plot(const antenna_pattern_t& pattern,
                               float elevation_deg,
                               const viz_params_t& params) -> void {
    ImVec2 canvas_size = ImGui::GetContentRegionAvail();
    canvas_size.y = canvas_size.x; // Make it square
    if (canvas_size.x < 100.0f) canvas_size.x = 400.0f;
    if (canvas_size.y < 100.0f) canvas_size.y = 400.0f;
