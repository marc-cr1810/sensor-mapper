...
public:
  ...
  static auto load_from_file(const std::string &filepath, antenna_file_format_t format) -> io_result_t
  {
    io_result_t result;
  ...
  static auto save_to_file(const antenna_pattern_t &pattern, const std::string &filepath, antenna_file_format_t format = antenna_file_format_t::CSV) -> io_result_t
  {
    io_result_t result;
...
private:
  ...
  static auto load_csv(const std::string &filepath) -> io_result_t
  {
    ...
    while (std::getline(file, line))
    {
      ...
      if (reading_header && tokens.size() >= 2)
      {
        ...
        {
          if (tokens.size() > 1)
            result.pattern->name = trim(tokens[1]);
          continue;
        }
        else if (tokens[0] == "Manufacturer")
        {
          if (tokens.size() > 1)
            result.pattern->manufacturer = trim(tokens[1]);
          continue;
        }
        else if (tokens[0] == "Model")
        {
          if (tokens.size() > 1)
            result.pattern->model = trim(tokens[1]);
          continue;
        }
        else if (tokens[0] == "Frequency_MHz")
        {
          if (tokens.size() > 1)
            result.pattern->frequency_mhz = std::stof(tokens[1]);
          continue;
        }
        else if (tokens[0] == "Max_Gain_dBi")
        {
          if (tokens.size() > 1)
            result.pattern->max_gain_dbi = std::stof(tokens[1]);
          continue;
        }
  ...
  // Save to CSV format
  static auto save_csv(const antenna_pattern_t &pattern, const std::string &filepath) -> io_result_t
  {
    io_result_t result;
    ...
    for (size_t elev_idx = 0; elev_idx < pattern.elevation_angles.size(); ++elev_idx)
    {
      ...
      {
        if (elev_idx < pattern.gain_db.size() && azi_idx < pattern.gain_db[elev_idx].size())
        {
          file << pattern.azimuth_angles[azi_idx] << "," << pattern.elevation_angles[elev_idx] << "," << pattern.gain_db[elev_idx][azi_idx] << "\n";
        }
      }
    }
  ...
  // Save to JSON format
  static auto save_json(const antenna_pattern_t &pattern, const std::string &filepath) -> io_result_t
  {
    io_result_t result;
    ...
    for (size_t elev_idx = 0; elev_idx < pattern.gain_db.size(); ++elev_idx)
    {
      ...
      file << "]";
      if (elev_idx < pattern.gain_db.size() - 1)
        file << ",";
      file << "\n";
    }
    file << "  ]\n";
  ...
  static auto organize_pattern_data(std::shared_ptr<antenna_pattern_t> &pattern, const std::vector<float> &azimuths, const std::vector<float> &elevations, const std::vector<float> &gains) -> void
  {
    if (azimuths.empty())
      return;
  ...
  static auto extract_json_string(const std::string &json, const std::string &key) -> std::string
  {
  ...
  static auto extract_json_float(const std::string &json, const std::string &key) -> float
  {
    std::string search = "\"" + key + "\"";
    size_t pos = json.find(search);
    if (pos == std::string::npos)
  ...
  static auto extract_json_array(const std::string &json, const std::string &key) -> std::vector<float>
  {
