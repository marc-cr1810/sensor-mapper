...
public:
  ...
  static auto generate_polar_plot(const antenna_pattern_t &pattern, float elevation_deg = 0.0f, const viz_params_t &params = viz_params_t()) -> polar_plot_data_t
  {
    ...
    for (int angle = 0; angle < 360; angle += params.resolution_deg)
    {
      ...
      {
        // dB scale
        point.radius = (point.gain_db - params.min_gain_db) / (params.max_gain_db - params.min_gain_db);
      }
  ...
  static auto generate_elevation_cut(const antenna_pattern_t &pattern, float azimuth_deg = 0.0f, const viz_params_t &params = viz_params_t()) -> pattern_cut_t
  {
    pattern_cut_t cut;
  ...
  static auto generate_azimuth_cut(const antenna_pattern_t &pattern, float elevation_deg = 0.0f, const viz_params_t &params = viz_params_t()) -> pattern_cut_t
  {
    pattern_cut_t cut;
  ...
  static auto generate_3d_mesh(const antenna_pattern_t &pattern, const viz_params_t &params = viz_params_t()) -> pattern_3d_mesh_t
  {
    ...
    for (int elev_idx = 0; elev_idx < elev_steps; ++elev_idx)
    {
      ...
      for (int azi_idx = 0; azi_idx < azi_steps; ++azi_idx)
      {
        ...
        {
          float norm_gain = (gain_db - params.min_gain_db) / (params.max_gain_db - params.min_gain_db);
  ...
  static auto gain_to_color(float gain_db, float min_db, float max_db, bool use_heatmap = true) -> gain_color_t
  {
    // Normalize to 0-1
  ...
  static auto get_3db_contour(const antenna_pattern_t &pattern, float elevation_deg = 0.0f) -> std::vector<float>
  {
  ...
  static auto generate_comparison_plot(const std::vector<std::shared_ptr<antenna_pattern_t>> &patterns, float elevation_deg = 0.0f, const viz_params_t &params = viz_params_t()) -> std::vector<polar_plot_data_t>
  {
  ...
  static auto generate_map_overlay(const antenna_pattern_t &pattern, double /* center_lat */, double /* center_lon */, float range_meters, float azimuth_offset_deg = 0.0f, const viz_params_t &params = viz_params_t()) -> map_overlay_data_t
  {
    ...
    auto center_color = gain_to_color(pattern.max_gain_dbi, params.min_gain_db, params.max_gain_db);
    ...
    for (int i = 0; i <= num_segments; ++i)
    {
      ...
      float radius_norm = (gain_db - params.min_gain_db) / (params.max_gain_db - params.min_gain_db);
