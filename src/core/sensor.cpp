...
sensor_t::sensor_t(const std::string &name, double latitude, double longitude, double range)
    : m_id(generate_id()), m_name(name), m_latitude(latitude), m_longitude(longitude), m_range(range), m_mast_height(10.0), m_ground_elevation(0.0), m_use_auto_elevation(true), m_locked(false), m_color({1.0f, 0.0f, 0.0f}),
      ...
      m_azimuth_deg(0.0), m_beamwidth_deg(360.0), m_propagation_model(PropagationModel::FreeSpace)
{
  // Random color logic...
  m_color = {static_cast<float>(rand()) / static_cast<float>(RAND_MAX), static_cast<float>(rand()) / static_cast<float>(RAND_MAX), static_cast<float>(rand()) / static_cast<float>(RAND_MAX)};
}

auto sensor_t::get_name() const -> const std::string &
{
  return m_name;
}

auto sensor_t::set_name(const std::string &name) -> void
{
  m_name = name;
}

auto sensor_t::get_latitude() const -> double
{
  return m_latitude;
}

auto sensor_t::set_latitude(double latitude) -> void
{
  m_latitude = latitude;
}

auto sensor_t::get_longitude() const -> double
{
  return m_longitude;
}
...
auto sensor_t::get_range() const -> double
{
  return m_range;
}

auto sensor_t::set_range(double range) -> void
{
  m_range = range;
}

auto sensor_t::get_mast_height() const -> double
{
  return m_mast_height;
}
...
auto sensor_t::get_color_data() -> float *
{
  return m_color.data();
}

auto sensor_t::get_color() const -> std::array<float, 3>
{
  return m_color;
}

auto sensor_t::get_id() const -> const std::string &
{
  return m_id;
}
auto sensor_t::set_id(const std::string &id) -> void
{
  m_id = id;
}

// RF Propagation Parameter Accessors
auto sensor_t::get_tx_power_dbm() const -> double
{
...
auto sensor_t::get_frequency_mhz() const -> double
{
...
auto sensor_t::get_azimuth_deg() const -> double
{
...
auto sensor_t::get_beamwidth_deg() const -> double
{
  return m_beamwidth_deg;
}
...
auto sensor_t::set_custom_pattern(std::shared_ptr<antenna_pattern_t> pattern) -> void
{
  m_pattern = pattern;
}
...
auto sensor_t::get_antenna_gain(double azimuth_deg, double elevation_deg) const -> double
{
  ...
  if (m_pattern && !m_pattern->gain_db.empty() && !m_pattern->gain_db[0].empty())
  {
    ...
    return m_pattern->get_gain(static_cast<float>(rel_angle), static_cast<float>(elevation_deg), true);
  }
