...
public:
  ...
  static auto generate_cellular_sector_antenna(float frequency_mhz = 2100.0f,
                                                float electrical_tilt_deg = 0.0f)
  ...
  static auto generate_small_cell_omni(float frequency_mhz = 2400.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
  ...
  static auto generate_massive_mimo_array(int num_elements_h = 8, 
                                          int num_elements_v = 4,
                                          float frequency_mhz = 3500.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
    pattern->name = "Massive MIMO " + std::to_string(num_elements_h) + "x" + 
                    std::to_string(num_elements_v);
    pattern->manufacturer = "Generic";
    pattern->frequency_mhz = frequency_mhz;
    ...
    for (size_t elev_idx = 0; elev_idx < pattern->elevation_angles.size(); ++elev_idx) {
      ...
      for (int azi = 0; azi < 360; ++azi) {
        ...
        float h_pattern = calculate_array_factor(azi_angle, num_elements_h, 
                                                  pattern->horizontal_beamwidth_deg);
        float v_pattern = calculate_array_factor(elev, num_elements_v, 
                                                  pattern->vertical_beamwidth_deg);
  ...
  static auto generate_wifi_patch_panel(float beamwidth_deg = 80.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
  ...
  static auto generate_lora_gateway_collinear(int num_sections = 4)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
  ...
  static auto generate_iot_chip_antenna() -> std::shared_ptr<antenna_pattern_t> {
    ...
    for (size_t elev_idx = 0; elev_idx < pattern->elevation_angles.size(); ++elev_idx) {
      ...
      for (int azi = 0; azi < 360; ++azi) {
        // Add some variation
        float variation = 3.0f * std::sin(azi * M_PI / 180.0f) * 
                         std::cos(elev * M_PI / 180.0f);
  ...
  static auto generate_satellite_dish(float diameter_m, float frequency_ghz = 12.0f)
      -> std::shared_ptr<antenna_pattern_t> {
  ...
  static auto generate_microwave_backhaul(float diameter_m = 0.6f, 
                                          float frequency_ghz = 23.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    float wavelength_m = 0.3f / frequency_ghz;
  ...
  // Phased array radar
  static auto generate_phased_array_radar(int num_elements = 64, 
                                          float frequency_ghz = 10.0f,
                                          float scan_angle_deg = 0.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
    ...
    for (size_t elev_idx = 0; elev_idx < pattern->elevation_angles.size(); ++elev_idx) {
      ...
      for (int azi = 0; azi < 360; ++azi) {
        ...
        float h_pattern = calculate_phased_array_pattern(azi_angle, num_elements, 
                                                         pattern->horizontal_beamwidth_deg);
        float v_pattern = calculate_phased_array_pattern(elev, num_elements, 
                                                         pattern->vertical_beamwidth_deg);
  ...
  static auto generate_beamformed_pattern(float beam_azimuth_deg, 
                                          float beam_elevation_deg,
                                          float beamwidth_deg = 15.0f,
                                          int num_elements = 16)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
  ...
  static auto generate_multibeam_mimo(const std::vector<float>& beam_azimuths,
                                      float beamwidth_deg = 20.0f)
      -> std::shared_ptr<antenna_pattern_t> {
    auto pattern = std::make_shared<antenna_pattern_t>();
...
private:
  ...
  static auto calculate_array_factor(float angle_deg, int num_elements, 
                                     float /* beamwidth_deg */) -> float {
  ...
  static auto calculate_phased_array_pattern(float angle_deg, int num_elements,
                                             float beamwidth_deg) -> float {
    // Main beam
