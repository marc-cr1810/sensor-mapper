...
void shader_t::set_float_array(const std::string &name, int count, const float *values)
{
  glUniform1fv(get_uniform_location(name), count, values);
}

void shader_t::set_vec4_array(const std::string &name, int count, const float *values)
{
...
unsigned int shader_t::compile_shader(unsigned int type, const std::string &source)
{
